...
type Props = { evaluation: Evaluation | "pending"; rubric: Criterion[]; awardedXp: number; onTryAgain: () => void; busy: boolean; error: string; nextSlug?: string; successTitle?: string; pendingTitle?: string; isReplay?: boolean };

/** Practice replays keep the banked XP from the first pass, so they award nothing extra. */
export function ReplayBadge() {
  return <p className="mt-4 inline-flex items-center gap-2 rounded-full bg-white/15 px-4 py-2 text-sm font-black text-white">↺ Practice replay — no XP this time</p>;
}
...
export default function EvaluationResults({ evaluation, rubric, awardedXp, onTryAgain, busy, error, nextSlug, successTitle = "Well done!", pendingTitle = "Thanks for the pitch!", isReplay = false }: Props) {
