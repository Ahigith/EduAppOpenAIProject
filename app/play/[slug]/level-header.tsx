...
import { useLevelPhase, useReadLevelDraft } from "./level-phase";
...
export default function LevelHeader({ title, slug, practicing = false }: { title: string; slug: string; practicing?: boolean }) {
  const router = useRouter();
  const phase = useLevelPhase();
  const readDraft = useReadLevelDraft();
  ...
  /**
   * Checkpoints this level as in_progress and stores the player's unscored work so
   * they resume where they left off. Never touches XP — a completed level keeps its badge.
   */
  async function save(): Promise<boolean> {
    ...
    try {
      const response = await fetch("/api/progress/checkpoint", {
        ...
        body: JSON.stringify({ levelSlug: slug, status: "in_progress", ...(draft ? { draft } : {}) }),
      });
      ...
      const data = await response.json() as { status?: string; draftSaved?: boolean };
      show("ok", data.draftSaved ? "Progress saved — you can pick this up right where you left off." : data.status === "completed" ? "Progress saved — this level is already complete." : "Progress saved.");
      return true;
  ...
  return <>
    <header className="fixed inset-x-0 top-0 z-40 border-b border-[#5b3024] bg-[#3b2019]/95 backdrop-blur">
      <div className="mx-auto flex h-16 max-w-4xl items-center justify-between gap-3 px-4 sm:px-8">
        ...
        <p className="hidden min-w-0 flex-1 items-center justify-center gap-2 truncate text-center text-sm font-bold text-[#f7e9d5] sm:flex">{title}{practicing ? <span className="shrink-0 rounded-full bg-white/15 px-2.5 py-0.5 text-xs font-black uppercase tracking-wider text-[#f7c65a]">↺ Practising</span> : null}</p>
    ...
    {confirming ? <div className="fixed inset-0 z-50 grid place-items-center bg-[#3b2019]/60 px-4 backdrop-blur-sm" onClick={(event) => { if (event.target === event.currentTarget) setConfirming(false); }}>
      <div aria-labelledby="leave-title" aria-modal="true" className="w-full max-w-md rounded-3xl border border-[#eadbca] bg-white p-6 shadow-2xl sm:p-8" role="dialog">
        ...
        <p className="mt-3 text-sm leading-6 text-[#765343]">{practicing ? "Your badge and XP for this level are already banked — practising never changes them. Saving keeps this practice run so you can carry on later." : "Saving keeps the work on this level so you can pick it up exactly where you left off."}</p>
